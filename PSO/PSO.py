...
# Edited by wanglei ：带线性递减惯性权重的PSO
...
class PSO(SkoBase):
    ...
    .. math::
       ...
       v_{ij}(t + 1) = w_new * v_{ij}(t) + c_{p}r_{1j}(t)[y_{ij}(t) − x_{ij}(t)]
    ...
    def update_W(self, iter):
        # 定义惯性权重线性衰减
        self.w = self.w0 - (self.w0 - self.w_min) * iter / self.max_iter

    def update_V(self):
        r1 = np.random.rand(self.pop, self.n_dim)
        r2 = np.random.rand(self.pop, self.n_dim)
        self.V = self.w * self.V + self.cp * r1 * (
            self.pbest_x - self.X) + self.cg * r2 * (self.gbest_x - self.X)
